...
namespace test
{
SCENARIO("Test the UtlruCache's insertion and deletion order.", "[cappuccino][UtlruCache]")
{
    GIVEN("A filled UtlruCache of 5 doubles with 1s TTL")
    {
        std::chrono::seconds                        ttl{1};
        cappuccino::UtlruCache<std::string, double> cache{ttl, 5};

        bool inserted1 = cache.Insert("one", 1.0); // first insert
        bool inserted2 = cache.Insert("two", 2.0);
        bool inserted3 = cache.Insert("three", 3.0);
        bool inserted4 = cache.Insert("four", 4.0);
        bool inserted5 = cache.Insert("five", 5.0); // last insert
        ...
        {
            bool inserted6 = cache.Insert("six", 6.0);
...
SCENARIO("The UtlruCache can store values in a limited space with both TTL and LRU disposal", "[cappuccino][UtlruCache]")
{
    using CacheType = cappuccino::UtlruCache<std::string, double>;
    std::chrono::seconds ttl{1};

    GIVEN("A filled UtlruCache of 5 doubles with 1s TTL")
    {
        CacheType cache{ttl, 5};
        ...
        WHEN("We access the last resource used")
        {
            ...
            {
                bool insert = cache.Insert("five", 5.0);
        ...
        {
            bool first_insert  = cache.Insert("persist", 1.0);
            auto first_access  = cache.Find("persist");
            bool test_ret_val1 = cache.Insert("persist", 1337.1337);
            bool test_ret_val2 = cache.Insert("persist", 1234.1234);
        ...
        {
            bool result1 = cache.Insert("one", 12.3);

            std::this_thread::sleep_for(ttl);

            bool result2 = cache.Insert("one", 45.6);
    ...
    GIVEN("A non-full UtlruCache using InsertOnly()")
    {
        CacheType cache{ttl, 5};
        ...
        WHEN("We insert keys in cache")
        {
            ...
            {
                REQUIRE(cache.InsertOnly("one", 1.0));
                REQUIRE_FALSE(cache.InsertOnly("one", 2.0));
       ...
       WHEN("We wait 1s for entry to expire")
       {
            ...
            {
                REQUIRE(cache.InsertOnly("one", 2.0));
                REQUIRE_FALSE(cache.InsertOnly("one", 3.0));
