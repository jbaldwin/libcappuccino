...
SCENARIO("Test the UtlruCache's insertion and deletion order.", "[cappuccino][UtlruCache]")
{
    GIVEN("A filled UtlruCache of 5 doubles with 1s TTL")
    {
        ...
        cache.Insert("one", 1.0); // first insert
        cache.Insert("two", 2.0);
        cache.Insert("three", 3.0);
        cache.Insert("four", 4.0);
        cache.Insert("five", 5.0); // last insert

        WHEN("We try to add a 6th entry the first one entered should be deleted.")
        {
            cache.Insert("six", 6.0);
...
SCENARIO("The UtlruCache can store values in a limited space with both TTL and LRU disposal", "[cappuccino][UtlruCache]")
{
    ...
    GIVEN("A filled UtlruCache of 5 doubles with 1s TTL")
    {
        ...
        {
            cache.Insert("persist", 1.0);
            auto first_access  = cache.Find("persist");
            cache.Insert("persist", 1337.1337);
            cache.Insert("persist", 1234.1234);
        ...
        {
            cache.Insert("one", 12.3);

            std::this_thread::sleep_for(ttl);

            cache.Insert("one", 45.6);
