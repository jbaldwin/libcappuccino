...
TEST_CASE("Lfu example")
{
    ...
    lfu_cache<uint64_t, std::string> cache{2};
    ...
    auto bar2 = cache.find_with_use_count(2);
    REQUIRE_FALSE(bar2.has_value());

    auto foo3   = cache.find_with_use_count(1);
...
{
    lfu_cache<uint64_t, std::string> cache{4};
...
{
    lfu_cache<uint64_t, std::string> cache{1};

    // Insert counts as a use.
    cache.insert(1, "test1");

    // Find counts as a use.
    auto element = cache.find_with_use_count(1);
