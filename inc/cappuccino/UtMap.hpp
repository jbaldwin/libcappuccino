...
{
/**
 * Uniform time aware associative map for key value pairs. The associative map has no fixed size
 * and will dynamically resize as key values are added. The map is less performant than the cache
 * data structures, but offers unlimited size.
 * 
 * Each key value pair is evicted based on an global map TTL (time to live)
 * Expired key value pairs are evicted on the first Insert, Delete, Find, or CleanExpiredValues
 * call after the TTL has elapsed. The map may have O(N) performance during these operations if
 * N elements are reaching their TTLs and being evicted at once.
 * 
 * This map is sync aware and can be used concurrently from multiple threads safely.
 ...
 * @tparam SyncType By default this map is thread safe, can be disabled for maps specific
 *                  to a single thread.
 */
...
public:
    /**
     * A simple {KeyType, ValueType} struct that can be used with InsertRange function.
     */
    struct KeyValue {
        KeyValue(
            KeyType key,
    ...
    /**
     * @param uniform_ttl The uniform TTL for key values inserted into the map. 100ms default.
     */
    UtMap(std::chrono::milliseconds uniform_ttl = std::chrono::milliseconds{100});

    /**
     * Inserts or updates the given key value pair using the uniform TTL.  On update will 
     * reset the TTL.
     ...
     * appropriate values from the map.
     *
...
private:
    ...
    /// The keyed lookup data structure, the value is the KeyedElement struct which includes the value and an iterator to the associated m_ttl_list TTlElement.
...
UtMap<KeyType, ValueType, SyncType>::UtMap(
    std::chrono::milliseconds uniform_ttl)
{
    m_uniform_ttl = uniform_ttl;
}

template <typename KeyType, typename ValueType, Sync SyncType>
auto UtMap<KeyType, ValueType, SyncType>::Insert(
        const KeyType& key,
        ValueType value,
        Allow allow) -> bool
{
...
template <typename KeyType, typename ValueType, Sync SyncType>
template <typename RangeType>
auto UtMap<KeyType, ValueType, SyncType>::InsertRange(
        RangeType&& key_value_range,
        Allow allow) -> size_t
{
    size_t inserted { 0 };
...
template <typename KeyType, typename ValueType, Sync SyncType>
auto UtMap<KeyType, ValueType, SyncType>::Delete(
    const KeyType& key) -> bool
{
    std::lock_guard guard { m_lock };
    const auto now = std::chrono::steady_clock::now();
...
template <typename KeyType, typename ValueType, Sync SyncType>
template <template <class...> typename RangeType>
auto UtMap<KeyType, ValueType, SyncType>::DeleteRange(
    const RangeType<KeyType>& key_range) -> size_t
{
    size_t deleted { 0 };
    std::lock_guard guard { m_lock };
...
template <typename KeyType, typename ValueType, Sync SyncType>
auto UtMap<KeyType, ValueType, SyncType>::Find(
    const KeyType& key) -> std::optional<ValueType>
{
    std::lock_guard guard { m_lock };
    const auto now = std::chrono::steady_clock::now();
...
template <typename KeyType, typename ValueType, Sync SyncType>
template <typename RangeType>
auto UtMap<KeyType, ValueType, SyncType>::FindRangeFill(
    RangeType& key_optional_value_range) -> void
{
    std::lock_guard guard { m_lock };
    const auto now = std::chrono::steady_clock::now();
...
template <typename KeyType, typename ValueType, Sync SyncType>
auto UtMap<KeyType, ValueType, SyncType>::doDelete(
    KeyedIterator keyed_elements_position) -> void
{
...
template <typename KeyType, typename ValueType, Sync SyncType>
auto UtMap<KeyType, ValueType, SyncType>::doFind(
    const KeyType& key) -> std::optional<ValueType>
{
    const auto keyed_position = m_keyed_elements.find(key);
    if (keyed_position != m_keyed_elements.end()) {
...
template <typename KeyType, typename ValueType, Sync SyncType>
auto UtMap<KeyType, ValueType, SyncType>::doPrune(
    std::chrono::steady_clock::time_point now) -> size_t
{
    ...
    // Delete the keyed elements from the map. Not using doDelete to take advantage of iterator range delete for TTLs.
