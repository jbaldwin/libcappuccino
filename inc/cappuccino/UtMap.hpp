...
public:
   ...
   */
  auto Insert(const KeyType &key, ValueType value,
  ...
  auto InsertRange(RangeType &&key_value_range,
...
private:
  ...
  auto doInsertUpdate(const KeyType &key, ValueType &&value,
                      std::chrono::steady_clock::time_point expire_time,
                      Allow allow) -> bool;

  auto doInsert(const KeyType &key, ValueType &&value,
                std::chrono::steady_clock::time_point expire_time) -> void;

  auto doUpdate(KeyedIterator keyed_position, ValueType &&value,
                std::chrono::steady_clock::time_point expire_time) -> void;
...
template <typename KeyType, typename ValueType, Sync SyncType>
auto UtMap<KeyType, ValueType, SyncType>::Insert(const KeyType &key,
                                                 ValueType value, Allow allow)
    -> bool {
...
auto UtMap<KeyType, ValueType, SyncType>::InsertRange(
    RangeType &&key_value_range, Allow allow) -> size_t {
  size_t inserted{0};
  std::lock_guard guard{m_lock};
  const auto now = std::chrono::steady_clock::now();
  const auto expire_time = now + m_uniform_ttl;
...
template <typename KeyType, typename ValueType, Sync SyncType>
auto UtMap<KeyType, ValueType, SyncType>::Find(const KeyType &key)
...
auto UtMap<KeyType, ValueType, SyncType>::doInsertUpdate(
    ...
    std::chrono::steady_clock::time_point expire_time, Allow allow) -> bool {
  const auto keyed_position = m_keyed_elements.find(key);
  if (keyed_position != m_keyed_elements.end()) {
...
auto UtMap<KeyType, ValueType, SyncType>::doUpdate(
    KeyedIterator keyed_position, ValueType &&value,
    std::chrono::steady_clock::time_point expire_time) -> void {
...
template <typename KeyType, typename ValueType, Sync SyncType>
auto UtMap<KeyType, ValueType, SyncType>::doFind(const KeyType &key)
    -> std::optional<ValueType> {
  const auto keyed_position = m_keyed_elements.find(key);
  if (keyed_position != m_keyed_elements.end()) {
